...
public class RecipeRegistry {
    ...
    public static ItemStack[] getDrops(int itemDamage) {
        if (rangeCheck(itemDamage)) {
            ...
            for(int i = 0; i < stacks.length;i++)
                stacks[i].stackSize = getDropAmount(itemDamage)[i];
            return stacks;
        }
        return null;
    }
    ...
    public static Integer[] getDropAmount(int itemDamage) {
        ArrayList<Integer> amount = new ArrayList<Integer>();
        SeedCropRecipe r = getSeedRecipe(itemDamage);
        for(int i = 0; i < r.getDrop().length;i++){
            int min = r.getDropMin()[i];
            int max = r.getDropMax()[i]; 
            amount.add(rand.nextInt(max - min + 1) + min);
        }
     return (Integer[])amount.toArray();  
    }
