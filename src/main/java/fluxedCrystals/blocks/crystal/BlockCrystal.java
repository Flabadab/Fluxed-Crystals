...
public class BlockCrystal extends CrystalBase implements ITileEntityProvider {
    ...
    public void dropCropDrops(World world, int x, int y, int z, Integer[] integers) {
        TileEntityCrystal crop = (TileEntityCrystal) world.getTileEntity(x, y, z);
        if (world.getBlockMetadata(x, y, z) >= 7) {
            doDrop(crop, world, x, y, z, integers);
        }
    }
    ...
    private void doDrop(TileEntityCrystal crop, World world, int x, int y, int z, Integer[] itemMultiplier) {
        ItemStack[] drop = crop.getDrop();

        if (drop == null) {
            return;
        }
        for (int j = 0; j < itemMultiplier.length; j++)
            for (int i = 0; i < drop.length; i++) {
                if (ConfigProps.shardDrop) {
                    dropBlockAsItem(world, x, y, z, new ItemStack(FCItems.shard, drop[i].stackSize + itemMultiplier[j], crop.getIndex()));
                } else {
                    dropBlockAsItem(world, x, y, z, new ItemStack(drop[i].getItem(), drop[i].stackSize + itemMultiplier[j], drop[i].getItemDamage()));
                }
            }
    }
        ...
        if (stack != null && stack.getItem() instanceof ItemScythe) {
            if (stack.isItemEqual(new ItemStack(FCItems.scytheWood))) {
                if (world.rand.nextInt(4) == 0) {
                    ...
                    dropCropDrops(world, x, y, z, new Integer[]{0});
                }
            }
