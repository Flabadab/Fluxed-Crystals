package fluxedCrystals.client.gui.ManaManager;
...
import fluxedCrystals.network.MessageManaManager;
...
public class GUIManaManager extends GuiContainer {

	private TileEntityManaManager tile;

	public GUIManaManager(InventoryPlayer invPlayer, TileEntityManaManager tile2) {
		super(new ContainerManaManager(invPlayer, tile2));
	...
	public void actionPerformed(GuiButton button) {
		...
		case 0:
			if (!tile.canPlacePowerBlocks()) {
				...
				PacketHandler.INSTANCE.sendToServer(new MessageManaManager(tile.xCoord, tile.yCoord, tile.zCoord, 1));
			}
			break;
		case 1:
			if (!tile.canPlacePowerBlocks()) {
				...
				PacketHandler.INSTANCE.sendToServer(new MessageManaManager(tile.xCoord, tile.yCoord, tile.zCoord, 2));
			}
			break;
		...
		case 2:
			if (!tile.canPlacePowerBlocks()) {
				...
				PacketHandler.INSTANCE.sendToServer(new MessageManaManager(tile.xCoord, tile.yCoord, tile.zCoord, 3));
			}
			break;
		case 3:
			if (!tile.canPlacePowerBlocks()) {
				...
				PacketHandler.INSTANCE.sendToServer(new MessageManaManager(tile.xCoord, tile.yCoord, tile.zCoord, 4));
			}
			break;
