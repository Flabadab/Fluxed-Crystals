...
public class TileEntityManagerBlock extends TileEnergyBase implements IInventory {
    ...
    public void updateEntity() {
        if (!(lastTick == worldObj.getTotalWorldTime())) {
                    ...
                    if (worldObj.getTotalWorldTime() % 20 == 0) {
                        if (isUpgradeActive(new ItemStack(FCItems.upgradeAutomation)))
                            if (worldObj.getBlockMetadata(power.getCropTile(worldObj).xCoord, power.getCropTile(worldObj).yCoord, power.getCropTile(worldObj).zCoord) >= 7) {
                                power.getCrop(worldObj).dropCropDrops(worldObj, power.getCropTile(worldObj).xCoord, power.getCropTile(worldObj).yCoord, power.getCropTile(worldObj).zCoord, new Integer[]{0});
            ...
            List<EntityPlayer> players = worldObj.getEntitiesWithinAABB(EntityPlayer.class, AxisAlignedBB.getBoundingBox(xCoord - 10, yCoord - 5, zCoord - 10, xCoord + 10, yCoord + 5, zCoord + 10));
            for (EntityPlayer player : players) {
                player.addChatComponentMessage(new ChatComponentText(StatCollector.translateToLocal("fc.tick.time.off." + new Random().nextInt(8))));
            }
        }
