 ...
 */
public class TileEntityThaumicManager extends TileEntity implements IInventory, IManager {
	...
	public TileEntityThaumicManager() {
					...
					if (worldObj.getTotalWorldTime() % 20 == 0) {
						if (isUpgradeActive(new ItemStack(FCItems.upgradeAutomation)))
							if (worldObj.getBlockMetadata(power.getCropTile(worldObj).xCoord, power.getCropTile(worldObj).yCoord, power.getCropTile(worldObj).zCoord) >= 7) {
								...
								AspectSourceHelper.drainEssentia(power.getCropTile(worldObj), Aspect.CROP, ForgeDirection.UNKNOWN, 15);
							}
					}
					if (AspectSourceHelper.findEssentia(this, Aspect.CROP, ForgeDirection.UNKNOWN, 15))
	...
	public void placePowerBlocks(int size) {
		for (int x = (size - (size * 2)); x <= size; x++) {
			for (int z = (size - (size * 2)); z <= size; z++) {
				if (worldObj.getBlock(xCoord + x, yCoord, zCoord + z) == FCBlocks.managerThaumic) {
					// NO-OP
				} else {
	...
	public boolean isItemValidForSlot(int slot, ItemStack stack) {
		if (stack == new ItemStack(FCBlocks.powerBlock)) {
			return true;
		}
		return false;
	}
